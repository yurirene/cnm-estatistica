@if(!empty($delegado->id))
    {!! Form::model(
        $delegado,
        [
            'url' => route('dashboard.cn.federacao.delegado.update', $delegado->id),
            'method' => 'PUT',
            'files' => true
        ]
    ) !!}
@else
    {!! Form::open([
        'url' => route('dashboard.cn.federacao.delegado.store'),
        'method' => 'POST',
        'files' => true
    ]) !!}
@endif
...
<div class="row mt-3">
    <div class="col-md-6">
        <div class="form-group">
            ...
            <a href="{{ route('dashboard.cn.federacao.index') }}" class="btn btn-secondary">
                Cancelar
            </a>
        </div>
    </div>
