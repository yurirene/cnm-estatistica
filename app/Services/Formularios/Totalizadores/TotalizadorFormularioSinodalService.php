    ...
    public static function totalizador($id)
    {
        ...
        try {
            ...
            foreach ($formularios as $formulario) {
                $totalizador['aci']['ump_repassaram'] += (
                    isset($formulario->aci['ump_repassaram'])
                    ? intval($formulario->aci['ump_repassaram'])
                    : 0
                );
                $totalizador['aci']['ump_nao_repassaram'] += (
                    isset($formulario->aci['ump_nao_repassaram'])
                    ? intval($formulario->aci['ump_nao_repassaram'])
                    : 0
                );

                if (
                    isset($formulario->aci['repasse'])
                    && ($formulario->aci['repasse'] == 'S' || $formulario->aci['repasse'] == 'SIM')
                ) {
                ...
                $totalizador['estrutura']['ump_organizada'] += (
                    isset($formulario->estrutura['ump_organizada'])
                    ? intval($formulario->estrutura['ump_organizada'])
                    : 0
                );
                $totalizador['estrutura']['ump_nao_organizada'] += (
                    isset($formulario->estrutura['ump_nao_organizada'])
                    ? intval($formulario->estrutura['ump_nao_organizada'])
                    : 0
                );
                ...
                $totalizador['programacoes_locais']['social'] += (
                    isset($formulario->programacoes_locais['social'])
                    ? intval($formulario->programacoes_locais['social'])
                    : 0
                );
                $totalizador['programacoes_locais']['oracao'] += (
                    isset($formulario->programacoes_locais['oracao'])
                    ? intval($formulario->programacoes_locais['oracao'])
                    : 0
                );
                $totalizador['programacoes_locais']['evangelistica'] += (
                    isset($formulario->programacoes_locais['evangelistica'])
                    ? intval($formulario->programacoes_locais['evangelistica'])
                    : 0
                );
                $totalizador['programacoes_locais']['espiritual'] += (
                    isset($formulario->programacoes_locais['espiritual'])
                    ? intval($formulario->programacoes_locais['espiritual'])
                    : 0
                );
                $totalizador['programacoes_locais']['recreativo'] += (
            ...
            $totalizador['estrutura']['federacao_organizada'] = Federacao::where('sinodal_id', $id)
                ->where('status', 1)
                ->count();
            $totalizador['estrutura']['federacao_nao_organizada'] = Federacao::where('sinodal_id', $id)
                ->where('status', 0)
                ->count();
            $totalizador['estrutura']['umps_organizadas'] = Local::where('sinodal_id', $id)
                ->where('status', 1)
                ->count();
            $totalizador['estrutura']['umps_nao_organizadas'] = Local::where('sinodal_id', $id)
                ->where('status', 0)
                ->count();
